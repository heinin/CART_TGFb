...
make_data <- function(cluster){
  message("Creating inputs for cluster ", cluster)
  ...
  write.table(cellAnnotations, file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_cellAnnotations.txt"),
              sep= "\t", quote = F)
  
  write.table(round(singleCell.counts.matrix, digits=3), 
              file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_singleCell.counts.matrix"), quote=F, sep="\t")
}

# A function for running infercnv for each cluster
run_infercnv <- function(cluster){
  message("Running infercnv for cluster ", cluster)
  # Create the infercnv object
  # Note: need to save all the related files in the working directory
  infercnv_obj <- CreateInfercnvObject(raw_counts_matrix=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_singleCell.counts.matrix"),
                                       annotations_file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_cellAnnotations.txt"),
                                       delim="\t",
  ...
  infercnv_obj <- infercnv::run(infercnv_obj,
                                ...
                                out_dir=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_HMMi3_samplelevel_output/"),  # dir is auto-created for storing outputs
                                cluster_by_groups=F,   # group cells by Sample
                                denoise=T,
                                num_threads=8,
                                analysis_mode='cells', # c("samples", "subclusters", "cells")
                                no_plot=F,
                                HMM=T,
  ...
  saveRDS(infercnv_obj, file = paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_HMMi3_samplelevel.rds"))
  ...
  plot_cnv(infercnv_obj,
           ...
           output_filename=paste0("seurat_data_", cluster, "_HMMi3_samplelevel_res"),
           output_format="pdf",
           #k_obs_groups = 5,
           dynamic_resize=0)
  ...
  annot <- read.table(paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_cellAnnotations.txt"))
  #selected <- WhichCells(seurat_data, idents = cluster)
  ...
  seurat_subset <- infercnv::add_to_seurat(infercnv_output_path=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_HMMi3_samplelevel_output/"),
  ...
  saveRDS(seurat_subset, file = paste0("/scratch/hnatri/CART/infercnv/seurat_data_", cluster, "_infercnv_HMMi3_samplelevel_seurat.rds"))
  
  message("Finnished running for cluster ", cluster)
  
}

# Running for all clusters
clusters <- unique(seurat_data$seurat_clusters)

for(cluster in clusters){
  make_data(cluster)
  run_infercnv(cluster)
  message("Done running for ", cluster)
}
