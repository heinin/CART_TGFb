...
Idents(seurat_data) <- seurat_data$CellLine
...
# A function for creating the inputs for infercnv for each sample
make_data <- function(sample){
  message("Creating inputs for ", sample)

  selected <- WhichCells(seurat_data, idents = sample)
  seurat_subset <- subset(seurat_data, cells = selected)
  
  # Extract out the count matrix
  singleCell.counts.matrix <- LayerData(seurat_subset, layer = "counts.1", assay = "RNA")
  ...
  cellAnnotations <- as.data.frame(cbind(rownames(seurat_subset@meta.data),
                                         seurat_subset@meta.data$KO_WT))
  ...
  write.table(cellAnnotations, file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_persample_cellAnnotations.txt"),
              sep= "\t", quote = F)
  
  write.table(round(singleCell.counts.matrix, digits=3), 
              file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_persample_singleCell.counts.matrix"), quote=F, sep="\t")
}

# A function for running infercnv for each sample
run_infercnv <- function(sample){
  message("Running infercnv for sample ", sample)
  # Create the infercnv object
  # Note: need to save all the related files in the working directory
  infercnv_obj <- CreateInfercnvObject(raw_counts_matrix=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_persample_singleCell.counts.matrix"),
                                       annotations_file=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_persample_cellAnnotations.txt"),
                                       delim="\t",
  ...
  infercnv_obj <- infercnv::run(infercnv_obj,
                                ...
                                out_dir=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_HMMi3_persample_subclusters_output/"),  # dir is auto-created for storing outputs
                                cluster_by_groups=T,   # group cells by Sample
                                denoise=T,
                                num_threads=8,
  ...
  saveRDS(infercnv_obj, file = paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_HMMi3_persample_subclusters.rds"))
  ...
  plot_cnv(infercnv_obj,
           ...
           output_filename=paste0("seurat_data_", sample, "_HMMi3_subclusters_res"),
           output_format="pdf",
           #k_obs_groups = 5,
           dynamic_resize=0)
  ...
  annot <- read.table(paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_persample_cellAnnotations.txt"))
  ...
  seurat_subset <- infercnv::add_to_seurat(infercnv_output_path=paste0("/scratch/hnatri/CART/infercnv/seurat_data_", sample, "_HMMi3_persample_subclusters_output/"),
  ...
  saveRDS(seurat_subset,
          file = paste0("/scratch/hnatri/CART/infercnv/seurat_data_",
                        sample, "_infercnv_HMMi3_persample_subclusters_seurat.rds"))
  
  message("Finnished running for sample ", sample)
  
}

# Running for all samples
samples <- unique(seurat_data$CellLine)
samples <- c("HD680", "HD624", "HD562")
#samples <- c("HD562")

for(sample in samples){
  make_data(sample)
  run_infercnv(sample)
  message("Done running for ", sample)
}
